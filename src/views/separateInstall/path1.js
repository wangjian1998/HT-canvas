...
class drawSence {
   ...
   */
  initSite(width = 6,height = 6, fillStyle = 'pink') {
    const color = {1: 'pink', 2: 'blue', 3: 'red', 4: 'grey', 5: 'yellow', 6: 'red', 7: 'blue', 8: 'green'}
    const icon = {1: shebeiImg, 2: cdImg, 3: dtImg, 4: gzzImg, 5: mjImg}
    ...
    path.forEach(item => {
      ...
      if(type === 6 || type === 7 || type === 8) {
        this.drawPoint(item.x, item.y, 3, color[type])
      }else {
        this.drawRect(item.x - width / 2,item.y - height / 2, width, height, color[type])
        this.drawImg(icon[type] || '', item.x - width / 2,item.y - height / 2, width, height)
      }
      this.ctx.closePath()
    })
  }
   ...
   */
  initGrid(space = 10) {
    this.drawAxis(0, 0, this.canvas.width, 0, '#000') // 绘制y轴
    this.drawAxis(0, 0, 0, this.canvas.height, '#000') // 绘制x轴
    // 绘制箭头
    this.drawAxis(this.canvas.width, 0, this.canvas.width - 10, 4, '#000')
    this.drawAxis(0, this.canvas.height, 4, this.canvas.height - 10, '#000')

    // 绘制网格
    for(let i = 0;i < Number(this.canvas.width);i += space) {
      this.drawAxis(i, 0, i, this.canvas.height)
      this.drawAxis(0, i, this.canvas.width, i)
    }
  ...
  paint() {
    this.clear();
    this.ctx.translate(0 + this.offset.x,this.canvas.height / 2 + this.offset.y);
    this.ctx.rotate(Math.PI);
